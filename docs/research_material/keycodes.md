...
That is where the tables come from when they are regenerated (see
[below](#decided-no-generator-in-the-repo)).
...
### Decided: no generator in the repo

*2026-09-23.* The keycode tables are **plain C++ source, produced once and committed** — no
generator script, no build-time step, no vendored JSON, no network fetch. QMK adds a keycode
version roughly once or twice a year, and nobody yet knows how often Nazg will need to follow.
When a new version appears, the tables are regenerated by hand (in practice by Claude) from
QMK's merged JSON for that version, and the diff is reviewed like any other change. Tooling
can be added later if regeneration ever becomes frequent enough to hurt.
